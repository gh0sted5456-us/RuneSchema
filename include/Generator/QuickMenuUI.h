...
inline constexpr float NavRailWidth=96, DesignWidth=876, Width=780, Height=720;
inline constexpr float HorizontalFit=Width/DesignWidth;
inline constexpr int Columns=4, ItemsPerPage=20, MaxSelection=64, MaxDrops=16;
inline constexpr float GridX=20, GridY=194, CardWidth=201, CardHeight=72, CardPitchX=208, CardPitchY=78;
...
public:
    ...
    void ClampScroll() {
        for(int t=0;t<3;++t)scroll[t]=QuickDecorations::ClampPage(scroll[t],Filtered(t).size(),t==0?ItemsPerPage:9);
    ...
    void Page(int steps) {
        ...
        if(itemDetailsOpen)itemDetailsPage-=steps;
    ...
    Command GiveCommand() const {
        ...
        const int n=Integer(quantity,1,10000,"Quantity");
    ...
    void Activate(const std::string& action,const std::string& arg={}) {
        try {
            ...
            if(action=="rail") {
                ...
                if(selection.contains(arg))selection.erase(arg);
                else if(selection.size()<MaxSelection)selection.insert(arg);
    ...
    Frame Render(float mouseX=-1,float mouseY=-1) {
        ...
        const Tint ash{.006f,.008f,.009f,.94f},gold{.72f,.52f,.20f,1},muted{.73f,.69f,.61f,1},ink{.94f,.9f,.82f,1};
        std::string hoveredPath;
        const auto rect=[&](Rect r,Tint c){f.draws.push_back({Draw::Kind::Rectangle,r,{},c,18,false,{}});};
        const auto soft=[&](Rect r,Tint c,float inset=3.f){
            inset=std::min(inset,std::min(r.w,r.h)*.25f);
            rect({r.x+inset,r.y,r.w-2*inset,inset},c);rect({r.x,r.y+inset,r.w,r.h-2*inset},c);
            rect({r.x+inset,r.y+r.h-inset,r.w-2*inset,inset},c);
        };
        const auto panel=[&](Rect r,Tint fill){soft(r,gold,5);soft({r.x+1,r.y+1,r.w-2,r.h-2},fill,4);};
        ...
        const auto button=[&](Rect r,std::string label,std::string action,std::string arg="",bool enabled=true,bool active=false,float left=12.f){
            ...
            soft(r,active?Tint{.73f,.56f,.27f,1}:hover?gold:Tint{.16f,.17f,.17f,1});
            soft({r.x+1,r.y+1,r.w-2,r.h-2},active?Tint{.075f,.065f,.045f,1}:Tint{.018f,.021f,.021f,1});
        ...
        const auto field=[&](Rect r,const std::string& id,std::string placeholder=""){
            ...
            soft(r,active?gold:Tint{.15f,.16f,.16f,1},2);rect({r.x+1,r.y+1,r.w-2,r.h-2},{.012f,.015f,.015f,1});
        ...
        const auto icon=[&](const std::string& path,Rect r){
            ...
            else {soft(r,{.035f,.04f,.04f,1},4);text("?",r.x+r.w/2,r.y+r.h*.28f,26,muted,true);}
        };
        ...
        const auto badge=[&](const std::string&,Rect r,const std::string& fallback,float alpha=1.f){
            ...
            soft(r,{.42f,.31f,.14f,alpha},2);rect({r.x+2,r.y+2,r.w-4,r.h-4},{.025f,.027f,.025f,.96f*alpha});
            text(Shorten(fallback,3),r.x+r.w/2,r.y+r.h*.22f,std::max(10.f,r.h*.42f),Tint{.92f,.76f,.43f,alpha},true);
        };
        const auto boldText=[&](const std::string& value,float x,float y,float size,Tint color){text(value,x,y,size,color);text(value,x+1,y,size,color);};
        ...
        const auto iconButton=[&](Rect r,const std::string& texture,const std::string& fallback,const std::string& tooltip,const std::string& action,bool enabled=true){
            const bool hover=enabled&&r.Contains(mouseX,mouseY);soft(r,hover?gold:Tint{.16f,.17f,.17f,1},2);soft({r.x+1,r.y+1,r.w-2,r.h-2},Tint{.018f,.021f,.021f,1},2);
        ...
        const auto placard=[&](Rect r,const Entry& e,const std::string& action,const std::string& arg,bool selected=false,bool allowFavorite=true,bool allowDetails=false){
            ...
            const Tint edge=e.masterwork?masterwork:selected||hover?gold:Tint{.14f,.15f,.15f,1};
            soft(r,edge,4);const float line=e.masterwork?2.f:1.f;
            soft({r.x+line,r.y+line,r.w-2*line,r.h-2*line},{.018f,.021f,.021f,1},3);
            const bool compact=r.h<100;
            if(compact) {
                // Compact item placards flow edge-to-edge. Artwork occupies the
                // left edge; identity and native metadata share the right side.
                soft(r,selected?Tint{.10f,.085f,.04f,1}:hover?Tint{.045f,.05f,.05f,.99f}:Tint{.014f,.017f,.017f,.98f},3);
                icon(e.icon,{r.x+6,r.y+7,56,56});
                const auto name=e.name.empty()?std::string("Choose item"):e.name;
                text(Shorten(name,21),r.x+69,r.y+7,15,e.masterwork?Tint{1.f,.73f,.12f,1}:e.available?ink:muted);
                const Rect metadata{r.x+68,r.y+36,r.w-75,28};
                soft(metadata,{.018f,.021f,.019f,.94f},3);
                const auto type=itemTypeLabel(e);
                if(!e.categoryIcon.empty())icon(e.categoryIcon,{metadata.x+5,metadata.y+5,18,18});
                else badge({}, {metadata.x+5,metadata.y+5,18,18},Shorten(type,1));
                const bool hasPower=e.available&&std::isfinite(e.power)&&e.power>=0;
                if(!hasPower)text(Shorten(type,11),metadata.x+28,metadata.y+7,12,muted);
                if(hasPower)
                    powerMark(e.power,metadata.x+metadata.w-42,metadata.y+7,12);
                ...
                if(selected)text("+",r.x+48,r.y+3,17);
                if(!e.path.empty()) {
                    ...
                    badge(QuickDecorations::Texture(origin),{r.x+2,r.y+2,15,15},QuickDecorations::Label(origin));
                    if(allowFavorite) {
                        const Rect hit{r.x+r.w-28,r.y+1,27,27};const bool favorite=IsFavorite(e.path),overStar=hit.Contains(mouseX,mouseY);
                        badge(QuickDecorations::FavoriteBadge,{hit.x+5,hit.y+5,17,17},"*",favorite?1.f:overStar?.8f:.38f);
        ...
        const auto overlay=[&](const std::string& title){
            f.hits.clear();f.rightHits.clear();hoveredPath.clear();rect({0,0,DesignWidth,Height},{0,0,0,.84f});panel({16,28,748,664},ash);
            text(title,36,47,23,{.84f,.68f,.36f,1});iconButton({710,38,38,38},QuickDecorations::CloseBadge,"X","Back / close","close");
        };
        panel({0,0,DesignWidth,Height},ash);
        iconButton({20,10,40,40},QuickDecorations::RuneSchemaBadge,"RS","About RuneSchema","helpy-about");
        if(Rect{20,10,40,40}.Contains(mouseX,mouseY))hoveredPath="RuneSchema Helpy / "+helpyKey;
        iconButton({554,10,40,40},QuickDecorations::RefreshBadge,"R","Refresh catalogue","refresh",!busy&&!indexing);
        iconButton({598,10,40,40},indexing?QuickDecorations::StopBadge:QuickDecorations::FullScanBadge,indexing?"STOP":"SCAN",indexing?"Stop full scan":advancedRuntime?"Run full scan":"Enable Advanced Runtime to run a full scan",indexing?"cancel":"index",indexing||advancedRuntime&&!busy);
        iconButton({642,10,40,40},QuickDecorations::SettingsBadge,"S","Settings","helpy-settings");
        iconButton({686,10,40,40},QuickDecorations::CoverageBadge,"?","Catalogue coverage","coverage");
        iconButton({730,10,40,40},QuickDecorations::CloseBadge,"X","Close Helpy","close");
        ...
        if(t==0){
            const char* chips[]{"All types","Armour","Weapons","Food","Potions","Consumables","Quest items","Farming"};
            const char* labels[]{"ALL","ARMOUR","WEAPONS","FOOD","POTIONS","CONSUMABLES","QUEST ITEMS","FARMING"};
            const float widths[]{60,86,92,62,78,106,108,86};
            constexpr float chipWidth=720;float x=(Width-chipWidth)/2;
            for(int i=0;i<8;++i){button({x,154,widths[i],30},labels[i],"item-filter-chip",chips[i],true,itemType==chips[i]);x+=widths[i]+6;}
        }
        ...
        if(t==0&&!cloneTab) {
            ...
            text(std::to_string(selection.size())+" selected",278,642,17,muted);field({404,634,76,34},"quantity");
            ...
            for(int slot=0;slot<9;++slot){
                const size_t index=static_cast<size_t>(scroll[t]*9+slot);if(index>=matches.size())break;
                const auto& e=entries[matches[index]];const float y=196+slot*46.f;Rect r{20,y,738,42};
                soft(r,r.Contains(mouseX,mouseY)?gold:Tint{.055f,.06f,.06f,1});
                const auto kind=NodeKind(e,t);const auto favoriteKey=NodeFavoriteKey(e,t);
                text(Shorten(e.name,48),32,y+3,18,e.available?ink:muted);text(Shorten(e.path.empty()?e.id:e.path,63),32,y+25,15,muted);
                const auto origin=e.runtimeClone||e.runeSchemaManaged?QuickDecorations::Origin::RuneSchema:
                    e.declaredModded?QuickDecorations::Origin::Modded:QuickDecorations::ItemOrigin(e.path,e.cooked,false);
                const bool showMod=(QuickDecorations::ModPath(e.path)||e.declaredModded)&&origin==QuickDecorations::Origin::RuneSchema;
                const float bx=showMod?654.f:682.f;
                f.draws.push_back({Draw::Kind::Badge,{bx,y+10,24,24},QuickDecorations::Texture(origin),{1,1,1,1},16,false,QuickDecorations::Label(origin)});
                if(showMod)f.draws.push_back({Draw::Kind::Badge,{682,y+10,24,24},QuickDecorations::ModdedBadge,{1,1,1,1},16,false,"MOD"});
                if(e.nodeKind=="NPC") {
                    float traitX=showMod?626.f:654.f;
                    const auto trait=[&](bool present,const char* texture,const char* fallback){
                        if(!present)return;f.draws.push_back({Draw::Kind::Badge,{traitX,y+10,24,24},texture,{1,1,1,1},16,false,fallback});traitX-=28.f;
                    };
                    const bool specialized=e.npcVendor||e.npcQuestGiver||e.npcLore||e.npcDialogue;
                    trait(e.npcVendor,QuickDecorations::VendorBadge,"V");
                    trait(e.npcQuestGiver,QuickDecorations::QuestGiverBadge,"Q");
                    trait(e.npcLore,QuickDecorations::LoreBadge,"L");
                    trait(e.npcDialogue,QuickDecorations::DialogueBadge,"D");
                    trait(!specialized,QuickDecorations::NpcBadge,"NPC");
                }
                const Rect fav{710,y+4,36,36};
                f.draws.push_back({Draw::Kind::Badge,{716,y+10,24,24},QuickDecorations::FavoriteBadge,IsFavorite(favoriteKey)?ink:Tint{.55f,.53f,.48f,.6f},16,false,IsFavorite(favoriteKey)?"*":"+"});
                f.hits.push_back({r,"node",e.id});f.hits.push_back({fav,"node-favorite",favoriteKey});
                if(r.Contains(mouseX,mouseY))hoveredPath=(kind==HelpyNodes::Kind::OtherResource?"Other resource: ":"")+(e.path.empty()?e.detail:e.path);
            }
            pager(612,scroll[t],matches.size(),9);
            if(t==1)button({20,646,348,26},"Dismiss temporary NPCs","dismiss-npcs","",!busy&&catalog.authority);
            else button({20,646,416,26},std::string(includeOtherResources?"[x] ":"[ ] ")+"Other resources in Stone & Ore","other-resources");
        ...
        if(helpyAboutOpen){
            ...
            text("Helpy for RuneSchema 0.7.0",390,194,27,ink,true);
        ...
        // Always visible, including in pickers. Unknown totals never display a fake percentage.
        const Rect progress{8,694,764,20};
        soft(progress,{.035f,.04f,.04f,1},2);
        if(indexing&&!indexHasTotal) {
            const float at=static_cast<float>((progressFrame++/3)%81)/100.f;
            rect({progress.x+progress.w*at,progress.y,progress.w*.19f,progress.h},{.28f,.21f,.08f,1});
        }else {
            const double fraction=indexTotal?std::min(1.0,static_cast<double>(indexDone)/indexTotal):(indexFinished?1.0:0.0);
            rect({progress.x,progress.y,progress.w*static_cast<float>(fraction),progress.h},{.28f,.21f,.08f,1});
        }
        std::string summary=indexStage;
        if(indexing&&indexHasTotal)summary+="  "+std::to_string(indexDone)+" / "+std::to_string(indexTotal);
        summary+="  |  Items "+std::to_string(catalog.entries[0].size())+"  NPC/AI "+std::to_string(catalog.entries[1].size())+"  Resources "+std::to_string(catalog.entries[2].size());
        if(!favoritesWritable)summary="Favorites: session only. "+summary;
        text(Shorten(summary,92),Width/2,696,15,ink,true);
        ...
        const bool navBlocked=node.has_value()||ItemPickerOpen()||playerPicker||choicePicker||cloneFieldOpen||cloneAdvancedOpen||
            ...
            reportOpen||actionReportOpen||itemDetailsOpen||recipeOpen||!itemFilterMenu.empty();
        ...
        const auto railButton=[&](float y,const char* mark,const char* label,const char* arg,bool active){
            ...
            const Tint edge=active?gold:hover?Tint{.46f,.35f,.18f,1}:Tint{.13f,.14f,.14f,.9f};
            railRect({r.x,r.y,r.w,2},edge);railRect({r.x,r.y+r.h-2,r.w,2},edge);railRect({r.x,r.y,2,r.h},edge);railRect({r.x+r.w-2,r.y,2,r.h},edge);
            railRect({r.x+3,r.y+3,r.w-6,r.h-6},active?Tint{.045f,.04f,.025f,.97f}:hover?Tint{.045f,.047f,.044f,.97f}:Tint{.008f,.011f,.011f,.97f});
            railText(mark,r.x+r.w/2,r.y+7,22,active?Tint{.94f,.73f,.37f,1}:ink,true);
            railText(label,r.x+r.w/2,r.y+37,11,active?Tint{.94f,.73f,.37f,1}:muted,true);
            if(!navBlocked&&!busy)f.hits.push_back({r,"rail",arg});
        };
        railRect({0,0,NavRailWidth-4,Height},{.006f,.008f,.009f,.965f});railRect({NavRailWidth-5,0,1,Height},{.5f,.36f,.14f,.55f});
        railText("RS",NavRailWidth/2-2,15,27,gold,true);railText("HELPY",NavRailWidth/2-2,49,12,muted,true);
        railButton(82,"I","ITEMS","items",tab==Tab::Items&&!cloneTab);
        railButton(150,"N","NPC / AI","ai",tab==Tab::Enemies);
        railButton(218,"R","RESOURCES","resources",tab==Tab::Resources);
        railButton(286,"C","ITEM LAB","clone",tab==Tab::Items&&cloneTab);
        railButton(374,"S","SETTINGS","settings",helpySettingsOpen);
        railButton(442,"?","ABOUT","about",helpyAboutOpen);
        railText(helpyKey,NavRailWidth/2-2,660,12,muted,true);railText("RuneSchema",NavRailWidth/2-2,682,10,Tint{.45f,.46f,.45f,1},true);
