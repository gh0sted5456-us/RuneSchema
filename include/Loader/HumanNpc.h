...
inline void Validate(const nlohmann::json& data) {
    if(!IsHuman(data)) {
        if(data.contains("Appearance") || data.contains("Equipment") || data.contains("Pose") || data.contains("DialoguePose") || data.contains("Ghost"))throw std::runtime_error("Appearance/Equipment/Pose/DialoguePose/Ghost require Type Human");
