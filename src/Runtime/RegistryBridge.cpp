...
constexpr auto BuildIdentity = "0.7.0";
...
void RegistryBridge::ObserveAuthorityPost(UObject* source,UFunction* function) {
    if(!source || !function || source!=m_activeAuthorityGraph)return;
    const auto path=RC::to_string(source->GetClassPrivate()->GetPathName());
...
void RegistryBridge::Observe(UObject* source,UFunction* function) {
    ...
    if(source->GetClassPrivate()->GetPathName()==WorldBridgeClass) {
        ...
        if(name==FName(TEXT("OnRep_WorldStateRevision"),FNAME_Add)) {
            ...
            const auto envelope=nlohmann::json::parse(ReadString(source,contract.ActivationEnvelope));if(!envelope.is_object())throw std::runtime_error("World activation envelope is invalid");m_seenActivationRevision=revision;
        }
        return;
    }
    ...
    if(contract.Compact) {
        ...
        if(activation>m_seenActivationRevision) {
            ...
            PS::Log<LogLevel::Warning>(STR("Registry activation {} reached this client, but client-side PureVFX replay has no verified native spawn adapter; authoritative gameplay must be spawned by the server graph.\n"),
                PS::ToWideSafe(state["activation"]["key"].get<std::string>().c_str()));
        }
    ...
    if(name==FName(TEXT("OnRep_RegistryRevision"),FNAME_Add)) {
        ...
        PS::Log<LogLevel::Warning>(STR("Registry activation {} reached this client, but client-side PureVFX replay has no verified native spawn adapter; authoritative gameplay must be spawned by the server graph.\n"),
            PS::ToWideSafe(envelope["key"].get<std::string>().c_str()));
