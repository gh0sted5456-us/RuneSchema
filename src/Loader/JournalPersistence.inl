...
struct JournalPersistence {
    ...
    static std::set<std::string> ConfirmAbsent(const Owners& owners) {
        ...
        if(!std::filesystem::is_directory(mods))throw std::runtime_error("Mod directory unavailable; journal cleanup refused");
        ...
        for(const auto& [id,mod]:owners) {
            ...
            // Existing but disabled, malformed or failed mods are not absent.
            const std::u8string path(mod.begin(),mod.end());
            if(!std::filesystem::exists(mods/path))absent.insert(mod);
        }
        return absent;
    }
