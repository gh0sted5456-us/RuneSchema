    ...
    if(action.QuestAction=="Accept") {
        ...
        if(receipt.Phase()==2 || receipt.Phase()==3)result=QuestProgress::Result::Uncertain;
            ...
            if(receipt.Phase()==1 && staged.Complete()) {
                ...
                result=receipt.TurnIn([&]{return current() && state()=="Given" && rewardInventory.Ready(spec.Count);},current,
        ...
        result=receipt.TurnIn(
            [&]{return current() && state()=="Given" && native.GetInt(counter)==quest.Required.Count && rewardInventory.Ready(rewardSpec.Count);},
            [&]{return current();},
        ...
        result=receipt.TurnIn(
            [&]{return current() && state()=="Given" && inventory.Count()>=quest.Required.Count && rewardInventory.Ready(rewardSpec.Count);},
