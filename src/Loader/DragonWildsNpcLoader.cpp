...
#include "HelpyNpcAuthoring.inl"
        ...
        m_tickCallbackId = Hook::RegisterEngineTickPostCallback(
            [this](Hook::TCallbackIterationData<void>&, UEngine*, float deltaSeconds, bool) {
                ...
                if(std::ranges::any_of(m_definitions,[](const auto& definition){
                    return !definition.HelpyTemporary && definition.Enabled && definition.Time!=TimeOfDay::Requirement::Any;
                })) {
    ...
    {
        if(data.is_array()) { for(const auto& entry:data)LoadVendors(entry,modName); return; }
        ...
        const std::string& fallbackId, const std::string& storeOwner)
    {
        ...
        if (data.contains("Items"))
        {
            ...
            if(definition.ItemsProperty.empty())for(const auto& item:definition.Items) {
                (void)VendorOffers::Properties(item);
                (void)VendorOffers::Category(item);
            }
        ...
        for(const auto& rule:definition.CategoryRules)
            if(!std::ranges::any_of(definition.Items,[&](const auto& item){return VendorOffers::Category(item)==rule.Category;}))
                throw std::runtime_error("CategoryRules references a category with no offers: "+rule.Category);
    ...
    void DragonWildsNpcLoader::QueueQuestRefresh(UObject* controller,const std::string& quest,double delaySeconds,std::string toast)
    {
        ...
        if(found==m_pendingQuestRefreshes.end())m_pendingQuestRefreshes.push_back({PS::WeakObject(controller),quest,std::move(toast),due});
        else {found->Due=std::max(found->Due,due);if(!toast.empty())found->Toast=std::move(toast);}
    }
    ...
    void DragonWildsNpcLoader::PumpQuestRefreshes()
    {
        ...
        for(auto& request:pending) {
            ...
            if(!request.Toast.empty())try {m_events.Notify(controller,request.Toast,Events::Scope::Participant);}
            catch(const std::exception& error){ErrorOnce("quest-toast:"+request.Quest,PS::ToWideSafe(error.what()));}
        }
    }
        ...
        const VendorDefinition& definition,const nlohmann::json* visibleItems)
    {
        ...
        const auto& sourceItems=visibleItems?*visibleItems:definition.Items;
            ...
            if(itemsProperty) {
                ...
                for(const auto& item:sourceItems) {
                    ...
                    (*found)["Collection"].push_back({{"AssetPathName",RC::to_string(recipe->GetPathName())},{"SubPathString",""}});
                        ...
                        WarnOnce("vendor-offer:"+owner+":"+slot,RC::to_generic_string("Vendor '"+definition.Id+"' skipped offer '"+slot+"': "+error.what()));
                    }
                }
            }
            else
