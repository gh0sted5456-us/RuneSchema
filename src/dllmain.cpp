...
public:
    RuneSchema() : CppUserModBase()
    {
        ...
        ModVersion = STR("0.6.1E");
        ModDescription = STR("RuneSchema extended authoring edition with deterministic mod loading.");
        ModAuthors = STR("Snorkles (RuneSchema creator); Jonesing4Space (extended features); based on Okaetsu's PalSchema");
    ...
    auto render_overview() -> void
    {
        ...
        ImGui::Text("RuneSchema %s", "0.6.1E");
        ImGui::TextWrapped("Created by Snorkles; extended features by Jonesing4Space.");
