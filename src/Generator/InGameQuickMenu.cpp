...
UFunction* CanvasFunction(const TCHAR* path) {
    return UECustom::UObjectGlobals::StaticFindObject<UFunction*>(nullptr, nullptr, path, false);
}
...
void InGameQuickMenu::ResetWorld() {
    ...
    ReleaseViewportInput();m_ui.Reset();m_catalogAttempted=false;
...
void InGameQuickMenu::RenderCanvas(UObject* canvas) {
    ...
    const float factor=std::min({(screenW-24)/QuickUI::Width,(screenH-24)/QuickUI::Height,1.0f});
    ...
    SyncFavorites(m_ui);
    ReadCatalog();
    if(!m_catalogAttempted&&SpawnToolRequests::IsAvailable()&&!SpawnToolRequests::IsWaiting()) {
        m_catalogAttempted=true;QuickUI::Command refresh;refresh.kind=QuickUI::Command::Kind::Refresh;Submit(refresh);
    }
