...
def main() -> None:
    ...
    checks = [
        ...
        ('world reset requests index cancellation', 'SpawnToolRequests::CancelRequested.store(true)' in native),
        ...
        ('first F2 open loads the external catalog and saved caches remain diagnostic-only', 'refresh.kind=QuickUI::Command::Kind::Refresh;Submit(refresh)' in native and '(void)LoadSettings(full)' in index and 'if(diagnosticCatalogs)' in index and 'MetadataVersion' in index),
