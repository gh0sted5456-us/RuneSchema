...
struct ToolCatalogIndex {
    ...
    void Start(UWorld* current,nlohmann::json definitions,nlohmann::json recipients,bool full=false,bool updateReference=false) {
        ...
        // A normal Helpy refresh must stay responsive. Enumerating every
        // mounted FAssetData record is an explicit Full Scan operation only;
        // ordinary refresh validates bundled, cached, and already-loaded paths.
        if(fullScan || updateReference)
        ...
        if(!registryReady||updateReference) {
            ...
            // Network discovery is opt-in. Opening Helpy or pressing Refresh
            // must never trigger a large remote reference download; only the
            // dedicated Update RSDW command may replace/fetch that index.
            const bool fetch=updateReference&&PS::HelpyReferencePolicy::ShouldFetch(sources.onlineReference,referencePresent,
                referenceCacheWritable,g_helpyInitialReferenceAttempted,true);
            if(fetch) {
