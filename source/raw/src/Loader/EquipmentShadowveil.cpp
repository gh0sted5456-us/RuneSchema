...
EquipmentShadowveilStatus InitializeEquipmentShadowveil(const ShadowveilRules::Rules& rules) {
    ...
    if(ExecutableTimestamp && ExecutableImageSize)
        PS::Log<LogLevel::Normal>(TEXT("[CAPABILITY][equipment.shadowveil-native][inactive] {} (exe 0x{:08X}/0x{:X}); other equipment capabilities remain available.\n"),failure,ExecutableTimestamp,ExecutableImageSize);
    else PS::Log<LogLevel::Normal>(TEXT("[CAPABILITY][equipment.shadowveil-native][inactive] {}; other equipment capabilities remain available.\n"), failure);
