...
namespace DragonWilds {
    ...
    FFieldClass* PropertyHelper::FindFieldClassByName(const RC::StringType& Name)
    {
        ...
        return host&&host->FindFieldClass?static_cast<FFieldClass*>(host->FindFieldClass(Name.c_str())):nullptr;
#else
...
#ifdef RUNESCHEMA_PLUGIN_CLIENT
        const auto* host=PS::PluginRuntimeServices::Host;
        return host&&host->IsFieldA&&host->IsFieldA(Field,FieldClass)!=0;
#else
        return Field && FieldClass && Field->IsA(FieldClass);
#endif
    }
}
