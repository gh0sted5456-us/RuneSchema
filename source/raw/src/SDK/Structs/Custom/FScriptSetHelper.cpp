...
namespace UECustom {
    ...
    bool FScriptSetHelper::Remove(const void* ElementToRemove)
    {
        if (ScriptSet->Num() < 0)
            throw std::runtime_error("Failed to remove TSet entry due to invalid ScriptSet.");
        for (auto Index = 0; Index < ScriptSet->GetMaxIndex(); ++Index)
        {
            if (!ScriptSet->IsValidIndex(Index)) continue;
            void* element = ScriptSet->GetData(Index, SetLayout);
            if (!ElementProp->Identical(element, ElementToRemove)) continue;
            ElementProp->DestroyValue(element);
            ScriptSet->RemoveAt(Index, SetLayout);
            return true;
        }
        return false;
    }
