...
struct Member {std::string Spawn;std::array<double,3> Position;bool Ground=true,RelativeGround=false;double GroundOffset=0;};
        ...
        for(const auto& member:wave) {
            ...
            for(size_t i=0;i<3;++i) {
                if(i==2 && position[i].is_string()) {
                    ...
                    item.Position[i]=0;
                    // An explicit "$" means the traced surface itself. Member's
                    // default capsule-clearance offset only applies to numeric Z.
                    item.GroundOffset=0;
