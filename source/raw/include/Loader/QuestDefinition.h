...
struct ItemAmount { std::string Item; int Count; TimeOfDay::Requirement Time=TimeOfDay::Requirement::Any; };
...
inline ItemAmount ParseItem(const Json& data) {
    ...
    const auto time=data.contains("TimeOfDay")?TimeOfDay::Parse(Dialogue::Text(data.at("TimeOfDay"),16)):TimeOfDay::Requirement::Any;
    return {item,count.get<int>(),time};
}
inline Definition ParseDefinition(const std::string& mod,const Json& data,bool stageObjective) {
    ...
    Dialogue::Fields(data.at("Reward"),{"Item","Count","TimeOfDay"});
    ...
    if(data.contains("RepeatReward")) {
        ...
        Dialogue::Fields(data.at("RepeatReward"),{"Item","Count","TimeOfDay"});
    ...
    if(data.contains("ResultTiers")) {
        ...
        for(const auto& entry:tiers) {
            ...
            Dialogue::Fields(entry.at("Reward"),{"Item","Count","TimeOfDay"});
