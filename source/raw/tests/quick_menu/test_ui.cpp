...
int main() {
    ...
    Check(tiles.size()==12,"exactly twelve visible item cards are drawn");
    for(int row=0;row<4;++row)for(int col=0;col<3;++col){Check(tiles[row*3+col].box.y==tiles[row*3].box.y,"three across");if(col)Check(tiles[row*3+col].box.x>tiles[row*3+col-1].box.x,"even column progression");}
